...
db.collection("Rooms").doc(roomId).collection("Messages").orderBy("timestamp", "asc").onSnapshot(resultsnap=>{
  setmessages(resultsnap.docs.map(doc=>{
    return (doc.data)()}))
  ...
  <p>Last seen at {new Date(messages[messages.length - 1]?.timestamp?.toDate()).toUTCString()}</p>
      ...
      <div className="sidechat_body">
        {messages.map(message=>(
          ...
          {message.message}
        <span className="sidechat_time">{new Date(message.timestamp?.toDate()).toUTCString()}</span>
        </p>
        ))}
      </div>
